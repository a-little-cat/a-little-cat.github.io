...
---
# KMS激活
本文介绍KMS激活windows和office的原理,搭建教程,以及激活教程.

其中office和windows的激活原理相同.
<!--more-->
## KMS原理
...
## 搭建
搭建基于国外大佬提供的[KMS服务器端](https://github.com/Wind4/vlmcsd),国内大佬提供的[一键部署脚本](https://github.com/dakkidaze/one-key-kms),微软提供的[GVLK](https://docs.microsoft.com/en-us/windows-server/get-started/kmsclientkeys).

搭建需要根据服务器类型运行对应的脚本.本次搭建是基于google cloud的centos vps,使用的是**one-key-kms-centos.sh**
...
./kms.sh start
# 可接受参数 start | stop | restart | status
iptables -I INPUT -p tcp --dport 1688 -j ACCEPT
# 手动放行1688端口
```

有必要的话,可以根据操作系统设置该脚本自启.编辑/etc/rc.local文件，在exit 0（如果有）前面加上一句/root/kms.sh start.

## 激活
下载并安装vol或其它版本的windows和office.使用管理员权限运行以下命令

如果已经修改过key,需要设置回gvlk,以下为专业版的gvlk,可以根据实际情况修改为其它系统,完整的gvlk列表由[微软](https://docs.microsoft.com/en-us/windows-server/get-started/kmsclientkeys)提供.

如果安装完系统没有修改过key,无需设置key.
### windows
```sh
...
## GVLK列表
此列表可能久不更新,如不能使用可至[微软官网](https://docs.microsoft.com/en-us/windows-server/get-started/kmsclientkeys)查询.
### Windows 10
...
# 参考链接
* [百度百科](https://baike.baidu.com/item/KMS/66195)
* [搭建教程](https://moe.best/tutorial/linux-kms-server.html)
* [激活脚本](http://kms.cangshui.net/)